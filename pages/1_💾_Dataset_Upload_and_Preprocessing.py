# pages/1_💾_Dataset_Upload_&Preprocessing.py
...
file_format = st.selectbox(
    ...
    index=0,
    help="Select 'Auto-detect' to infer the format from the file extension."
)
...
if st.session_state.get('df') is not None:
    ...
    with st.form(key='preprocessing_form'):
        ...
        columns_to_drop = st.multiselect(
            ...
            help="Select any columns that are not needed (e.g., IDs)."
        )
    ...
    if submit_button:
        ...
        st.success("Selections saved! You can now proceed to Model Training.")
        ...
        st.info("Navigate to the **'⚙️ Training'** page from the top navigation bar to continue.", icon="➡️")
